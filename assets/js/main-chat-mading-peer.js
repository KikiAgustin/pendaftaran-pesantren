...
function getStateOfChat() {
	if (!instanse) {
		...
		$.ajax({
			...
			crossDomain: true,
			cache: false,
			url: "https://www.dfunstation.com/api4/android/index.php/member/chat-mading-peer/" + userid + "/" + localStorage.userid + "//" + chatid + "/?callback=?",
...
function updateChatx() {
	if (!instanse) {
		...
		var idx = getvar("id");
		if (idx != "") {
			var urlx = "https://www.dfunstation.com/api4/android/index.php/member/chat-mading-peer/" + userid + "/" + localStorage.userid + "/" + idx + "/" + chatid + "/?callback=?";
		} else {
			var urlx = "https://www.dfunstation.com/api4/android/index.php/member/chat-mading-peer/" + userid + "/" + localStorage.userid + "/0/" + chatid + "/?callback=?";
		}
		$.ajax({
			type: "POST",
			crossDomain: true,
	...
	if (!instanse) {
		...
		if (length <= maxLength + 1) {
			$.ajax({
				...
				success: function (data) {
					slide('mading_post_peer.html?id=' + id);
					//updateChat();
...
function sendChat(message, nickname) {
	...
	if ($.trim(chatid).length > 0) {
		if (isimage_ == "1") {
			...
			ft.upload(imageURI, encodeURI("https://www.dfunstation.com/api4/android/index.php/member/chat-media-mading-peer/" + localStorage.userid + "/?callback=?"), win, fail, options);
		} else if (isimage == "1") {
