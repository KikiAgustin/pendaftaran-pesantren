...
function getStateOfChat() {
	if(!instanse){
		...
		$.ajax({
			...
			crossDomain: true,
			cache: false,
			url: "https://www.dfunstation.com/api4/android/index.php/member/chat-group/"+userid+"/"+localStorage.userid+"//"+chatid+"/?callback=?",
...
function updateChatx() {
	...
	if(!instanse){
		...
		$.ajax({
			...
			crossDomain: true,
			cache: false,
			url: "https://www.dfunstation.com/api4/android/index.php/member/chat-group/"+userid+"/"+localStorage.userid+"/"+localStorage.lastchatid+"/"+chatid+"/?callback=?",
				...
				switch (value) {
					case "catch":	
						...
						$.ajax({
							...
							crossDomain: true,
							cache: false,
							url: "https://www.dfunstation.com/api4/android/index.php/member/chat-group/"+userid+"/"+localStorage.userid+"/"+id+"/"+chatid+"/?callback=?",
...
function sendChat(message, nickname) { 
	...
	if($.trim(chatid).length>0){
		if(isimage_=="1") {
			...
			ft.upload(imageURI, encodeURI("https://www.dfunstation.com/api4/android/index.php/member/chat-media-group/"+localStorage.userid+"/?callback=?"), win, fail, options);
		} else if(isimage=="1") {
